...
/***/ "./src/assets/lib/jsMind/jsmind.js":
/*!*****************************************!*\
  !*** ./src/assets/lib/jsMind/jsmind.js ***!
  \*****************************************/
...
(function ($w) {
  'use strict'; // set 'jsMind' as the library name.
  // __name__ should be a const value, Never try to change it easily.

  var __name__ = 'jsMind'; // library version

  var __version__ = '0.4.6'; // author

  var __author__ = 'hizzgdev@163.com'; // an noop function define
  ...
  var logger = typeof console === 'undefined' ? {
    ...
    warn: _noop,
    info: _noop
  } : console; // check global variables
  ...
  var $i = function $i(el) {
    return !!el && _typeof(el) === 'object' && el.nodeType === 1 && _typeof(el.style) === 'object' && _typeof(el.ownerDocument) === 'object';
  };
  ...
  jm.mind.prototype = {
    ...
    remove_node: function remove_node(node) {
      ...
      children.length = 0; // remove from parent's children
      ...
      delete this.nodes[node.id]; // clean all properties
  ...
  jm.format = {
    ...
    node_array: {
      ...
      _parse: function _parse(mind, node_array) {
        var df = jm.format.node_array;
        var narray = node_array.slice(0); // reverse array for improving looping performance
    ...
    freemind: {
      ...
      _load_node: function _load_node(mind, parent_id, xml_node) {
        ...
        var node_topic = xml_node.getAttribute('TEXT'); // look for richcontent
        ...
        if (node_topic == null) {
          ...
          for (var i = 0; i < topic_children.length; i++) {
            topic_child = topic_children[i]; //logger.debug(topic_child.tagName);
      ...
      _buildmap: function _buildmap(node, xmllines) {
        ...
        xmllines.push('TEXT=\"' + node.topic + '\">'); // store expanded status as an attribute

        xmllines.push('<attribute NAME=\"expanded\" VALUE=\"' + node.expanded + '\"/>'); // for attributes
  ...
  jm.prototype = {
    ...
    dblclick_handle: function dblclick_handle(e) {
      ...
      if (this.get_editable()) {
        var element = e.target || event.srcElement;
        var nodeid = this.view.get_binded_nodeid(element); // if (!!nodeid) { //elva注释掉了，2019.10.11
  ...
  jm.layout_provider.prototype = {
    ...
    _layout_direction_root: function _layout_direction_root() {
      var node = this.jm.mind.root; // logger.debug(node);
    ...
    layout_offset: function layout_offset() {
      ...
      this.bounds.w = 0 - this.bounds.e; //logger.debug(this.bounds.w);
    ...
    _layout_offset_subnodes_height: function _layout_offset_subnodes_height(nodes) {
      ...
      while (i--) {
        node = nodes[i];
        node._data.layout.offset_y += middle_height; //logger.debug(node.topic);
    ...
    get_node_point: function get_node_point(node) {
      ...
      var offset_p = this.get_node_offset(node); //logger.debug(offset_p);

      var p = {};
      p.x = offset_p.x + view_data.width * (node._data.layout.direction - 1) / 2;
      p.y = offset_p.y - view_data.height / 2; //logger.debug(p);
    ...
    get_node_point_out: function get_node_point_out(node) {
      ...
      if (pout_cache.x == -1 || pout_cache.y == -1) {
        if (node.isroot) {
          ...
          pout_cache.y = offset_p.y; //logger.debug('pout');
    ...
    get_min_size: function get_min_size() {
      ...
      for (var nodeid in nodes) {
        node = nodes[nodeid];
        pout = this.get_node_point_out(node); //logger.debug(pout.x);
  ...
  jm.view_provider.prototype = {
    ...
    _show: function _show() {
      ...
      this.show_nodes();
      this.show_lines(); //this.layout.cache_valid = true;
...
//# sourceMappingURL=14-6845f85e.js.map