...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
      ...
      this.$axios.post('userLogin', data, {
        ...
        switch (res.msg) {
          case 0:
            ...
            var _data = res.CurrentUserInfo;
            sessionStorage.setItem('user_id', _data.id);
            var tids = [];

            if (_data.user_trees.length > 0) {
              //tids
              for (var i = 0; i < _data.user_trees.length; i++) {
                tids.push(_data.user_trees[i].t_id.toString());
              }
              ...
              _this2.$store.commit('setOrigTids', JSON.parse(sessionStorage.getItem('getTidList')));

              _this2.$getApi.getAllTRoot(_this2.$store.state.tids).then(function (res) {
    ...
    getUserInfo: function getUserInfo() {
      ...
      this.$axios.post('getCurrentUserInfo', name).then(function (res) {
        var data = res.CurrentUserInfo;
        var tids = [];

        if (data.user_trees.length > 0) {
          //tids
          for (var i = 0; i < data.user_trees.length; i++) {
            tids.push(data.user_trees[i].t_id.toString());
          }

          sessionStorage.setItem('getTidList', JSON.stringify(tids));

          _this3.$store.commit('setTids', JSON.parse(sessionStorage.getItem('getTidList')));

          _this3.$store.commit('setOrigTids', JSON.parse(sessionStorage.getItem('getTidList')));

          _this3.$getApi.getAllTRoot(_this3.$store.state.tids).then(function (res) {
            if (res !== undefined && res.msg !== undefined && res.msg === 0) {
              var tRoot = res.t_root;
