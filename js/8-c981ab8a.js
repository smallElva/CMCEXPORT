...
function selectHeader(type) {
  ...
  switch (type) {
    ...
    case 12:
      //振动，晃度，倾角
      header = ['任务标识', '分析频率', '谱线数', '采样周期', '谱线分辨率', '过采样', '报警投票机制a', '报警投票机制b'
      /*'安装高度(m)',*/
      ];
      break;
    ...
    case 4:
      //包络
      header = ['任务标识', '滤波器', '分析频率', '谱线数', '采样周期', '谱线分辨率', '过采样', '数码管显示', '报警投票机制a', '报警投票机制b'];
      break;

    case 15:
      //冲击测点
      header = ['任务标识', '分析频率', '谱线数', '采样周期', '谱线分辨率', '过采样', '数码管显示', '报警投票机制a', '报警投票机制b'];
      break;

    case 8:
      //振动阶次
      header = ['任务标识', '采样周期', '采样点数', '过采样', '数码管显示', '报警投票机制a', '报警投票机制b'];
      break;

    case 9:
      //包络阶次、冲击阶次
      header = ['任务标识', '滤波器', '采样周期', '采样点数', '过采样', '数码管显示', '报警投票机制a', '报警投票机制b'];
      break;
    ...
    case 11:
      //倾覆
      header = ['任务标识', '位移单位', '分析频率', '谱线数', '采样周期', '谱线分辨率', '过采样', '传感器名称', '晃度X名称', '关联通道X', '自振频率1', '晃度Y名称', '关联通道Y', '自振频率2', '安装高度(m)', '报警投票机制a', '报警投票机制b'];
      break;

    case 13:
      //合成倾角
      header = ['分析频率', '谱线数', '采样周期', '谱线分辨率', '过采样', '传感器名称', '传感类型', '数据来源', '偏置角度', '基础直径', '倾角X名称', '关联通道X', '偏移量X', '从设备号X', '过程量类型X', '倾角Y名称', '关联通道Y', '偏移量Y', '从设备号Y', '过程量类型Y', '安装高度(m)', '报警投票机制a', '报警投票机制b'];
      break;
    ...
    case 10:
      ...
      '预警(低)',
      '报警(低)',*/
      '工程量输入范围高', '工程量输入范围低', '信号输入范围高', '信号输入范围低', '校正参数', '零点偏移', '数据地址', '数据类型', '过程量类型', '传感器类型', '从设备号'];
      break;

    case 18:
      //脱轨检测测点
      header = ['轨枕长度(mm)', '脱轨的冲击阈值(G)', '连续撞击次数'];
  }
...
function setHeaderFrequency(headers, type) {
  ...
  if (type == 3 || type == 15 || type == 4 || type == 8 || type == 16 || type == 9 || type == 6 || type == 11 || type == 12 || type == 13 || type == 17) {
    ...
    for (var i = 1; i <= 8; i++) {
      ...
      obj1.push.apply(obj1, ["\u9891\u6BB5".concat(i, "\u9009\u62E9"), "\u9891\u6BB5".concat(i, "-1\u540D\u79F0"), "\u9891\u6BB5".concat(i, "-1\u7C7B\u578B"), "\u9891\u6BB5".concat(i, "-2\u540D\u79F0"), "\u9891\u6BB5".concat(i, "-2\u7C7B\u578B"), "\u9891\u6BB5".concat(i, "\u8D77\u59CB\u503C(Hz)"), "\u9891\u6BB5".concat(i, "\u7ED3\u675F\u503C(Hz)")]);
    } // obj1.push('')
    // headers[0].push(...obj0)
...
function selectColumns(type, data) {
  ...
  switch (type) {
    ...
    case 10:
      //工艺量
      col = [{
        data: "baseInfo.cc_mod_id_val",
        editor: 'select',
        selectOptions: function selectOptions(row) {
          return data[row].baseInfo.cc_mod_id_sources;
        }
      }, {
      ...
      { data: `baseInfo.ll_limit` },*/
      {
        ...
        data: "baseInfo.datatype_val",
        editor: 'select',
        selectOptions: function selectOptions(row) {
          return data[row].baseInfo.datatype_sources;
        }
      }, {
    ...
    case 18:
      col = [{
        data: "baseInfo.sleeper_len" //轨枕长度

      }, {
        data: "baseInfo.shock_threshold",
        //脱轨的冲击阈值
        editor: 'select',
        selectOptions: function selectOptions(row) {
          return data[row].baseInfo.shock_threshold_sources;
        }
      }, {
        data: "baseInfo.shock_num" //连续脱轨事件次数
...
function setColumnFrequency(col, type) {
  if (type == 3 || type == 15 || type == 4 || type == 8 || type == 16 || type == 9 || type == 6 || type == 11 || type == 12 || type == 13 || type == 17) {
    var optfreq_type_source = [];
    var band_factor_type_source = [];

    if (type == 17) {
      optfreq_type_source = ['有效值'];
      band_factor_type_source = ['有效值'];
    } else {
      optfreq_type_source = ['能量有效值', '速度值', '加速度值'];
      band_factor_type_source = ['频段因数', '速度值', '加速度值'];
    }
    ...
    for (var i = 0; i <= 7; i++) {
      col.push.apply(col, [{
        ...
        editor: 'select',
        selectOptions: optfreq_type_source
      }, {
        data: "baseInfo.band".concat(i, ".band_factor_name")
      }, {
        data: "baseInfo.band".concat(i, ".band_factor_type_val"),
        editor: 'select',
        selectOptions: band_factor_type_source
      }, {
...
var NORMAL_POS = [1, 2, 7, 5, 17, 18]; //转速、过程量、数组量、温度、轨道波磨、脱轨检测测点
//表格的方法
  ...
  _createClass(tableData, [{
          ...
          items: {
            "row_above": {
              ...
              callback: function callback(key, options) {
                Object(_pos_methods_js__WEBPACK_IMPORTED_MODULE_3__["addRows"])(that.getHot(), that.getData(), options[0].start.row, this.type);
              }
            },
            "remove_row": {
              name: '删除这行'
    ...
    value: function cells() {
      ...
      return function (row, col, prop) {
        ...
        var selectProps = ['t_name', 'mac_me', 'dgm_name', 'ch_name', 'baseInfo.method_val', 'pos_activity_val', 'baseInfo.capture_byId', 'baseInfo.capture_chName', 'isalarm_val', 'baseInfo.frequency_val', 'baseInfo.spectral', 'task_id_val', 'baseInfo.resampling_val', 'baseInfo.if_show_val', 'baseInfo.trendAlarm_val', 'baseInfo.filter_val', 'baseInfo.dip_model_val', 'baseInfo.sourceType_val', 'baseInfo.rock1_ch_name', 'baseInfo.datamode1_val', 'baseInfo.rock2_ch_name', 'baseInfo.datamode2_val', 'baseInfo.calc_method_val', 'baseInfo.datatype_val', 'baseInfo.datamode_val', 'baseInfo.sensor_id_val', 'baseInfo.shock_threshold', 'baseInfo.band0.optfreq_type_val', 'baseInfo.band1.optfreq_type_val', 'baseInfo.band2.optfreq_type_val', 'baseInfo.band3.optfreq_type_val', 'baseInfo.band4.optfreq_type_val', 'baseInfo.band5.optfreq_type_val', 'baseInfo.band6.optfreq_type_val', 'baseInfo.band7.optfreq_type_val', 'baseInfo.band0.band_factor_type_val', 'baseInfo.band1.band_factor_type_val', 'baseInfo.band2.band_factor_type_val', 'baseInfo.band3.band_factor_type_val', 'baseInfo.band4.band_factor_type_val', 'baseInfo.band5.band_factor_type_val', 'baseInfo.band6.band_factor_type_val', 'baseInfo.band7.band_factor_type_val', 'baseInfo.cc_mod_id_val'];
        ...
        if (type == 1) {
          ...
          if (row != -1 && prop == 'baseInfo.speedRate') {
        ...
        /*/!* 工艺量测点 *!/
        if (type == 10) {
          if (row != -1 && (prop == 'baseInfo.sinkData')) {//沉没度系数 只有油气井才需要输入，其他不可编辑
            if (data[row].t_root == 3) {
              cellProperties.readOnly = false;
            }else{
              cellProperties.readOnly = true;
            }
    ...
    value: function afterOnCellMouseUp() {
      ...
      return function (e, coords, td) {
        if (coords.row >= 0) {
          ...
          that.$store.commit('setTreePosition', linkObj); // console.log(row_info, coords.col);
        }
      };
    }
    /*过滤的方法(有待优化)（共用）*/
    ...
    value: function afterFilter() {
      ...
      return function (condition) {
        ...
        setTimeout(function () {
          ...
          hot.view.wt.wtOverlays.refreshAll();
        });
      };
    }
  }, {
    ...
    value: function afterScrollHorizontally() {
      ...
      return function () {
        ...
        var scrollWidth = wtHolder.scrollWidth;
        var wtWidth = wtHolder.clientWidth;
        var offset = scrollWidth - wtWidth - scrollLeft; // 滚动条距离最右边的距离
        // console.log(offset)

        if (offset < 21) {
          //当滚动条距离最右边的宽度小于20之后重新渲染，防止点击表格单元表头错行
          that.getHot().render();
        }
      };
    }
        ...
        for (var i = 0; i < changes.length; i++) {
          ...
          if (change_prop == "baseInfo.filter_val") {
            ...
            if (filter_sources.indexOf(change_newVal) == -1) {
              ...
              if (position.pos_type == 9) {
                //包络阶次测点修改滤波器不需要把采样点数和采样周期重置
                that.update_filter_frequency_spectral_sources(position);
              } else {
          ...
          if (change_prop == "baseInfo.frequency_val") {
            ...
            if (frequency_sources.indexOf(change_newVal) == -1) {
              ...
              position.baseInfo.frequency = Object(utils_utils__WEBPACK_IMPORTED_MODULE_5__["find_key"])(change_newVal, sources, keys); // 针对非超级用户，v1采集器 谱线数下拉选项过滤

              that.filter_spectral_sources(position);
          ...
          for (var _i3 = 0; _i3 < 8; _i3++) {
            ...
            if (change_prop == "baseInfo.band".concat(_i3, ".band_factor_type_val")) {
              //2频段
              var band_factor_type_sources = position.baseInfo["band".concat(_i3)].band_factor_type_sources;

              if (band_factor_type_sources.indexOf(change_newVal) == -1) {
                swal({
                  title: "提示",
                  text: "\u7B2C".concat(change_index + 1, "\u884C\u4E0D\u5B58\u5728\u8BE5\u9891\u6BB5\u7C7B\u578B"),
                  button: "确认"
                ...
                sources = position.baseInfo["band".concat(_i3)].band_factor_type_sources;
                keys = position.baseInfo["band".concat(_i3)].band_factor_type_sources_key;
                position.baseInfo["band".concat(_i3)].band_factor_type_val = change_newVal;
                position.baseInfo["band".concat(_i3)].band_factor_type = Object(utils_utils__WEBPACK_IMPORTED_MODULE_5__["find_key"])(change_newVal, sources, keys);
              }
            }
          }
          ...
          if (change_prop == "baseInfo.datatype_val") {
            //数据类型  工艺量测点
            var datatype_val_sources = position.baseInfo.datatype_sources;

            if (datatype_val_sources.indexOf(change_newVal) == -1) {
              swal({
                title: "提示",
                text: "\u7B2C".concat(change_index + 1, "\u884C\u4E0D\u5B58\u5728\u8BE5\u6570\u636E\u7C7B\u578B"),
                button: "确认"
              });
              return false;
            } else {
              sources = position.baseInfo.datatype_sources;
              keys = position.baseInfo.datatype_sources_key;
              position.baseInfo.datatype_val = change_newVal;
              position.baseInfo.datatype = Object(utils_utils__WEBPACK_IMPORTED_MODULE_5__["find_key"])(change_newVal, sources, keys);
            }
          }
          ...
          if (change_prop == "baseInfo.alarm_smooth_a") {
            //报警投票机制a，新增：正整数
            var alarm_b = Number(position.baseInfo.alarm_smooth_b);
            ...
            if (Number(change_newVal) > alarm_b) {
              swal({
                title: "提示",
                text: "\u7B2C".concat(change_index + 1, "\u884C\u62A5\u8B66\u6295\u7968a\u4E0D\u5F97\u5927\u4E8E\u62A5\u8B66\u6295\u7968b"),
                button: "确认"
              });
              return false;
            }
          ...
          if (change_prop == "baseInfo.alarm_smooth_b") {
            //报警投票机制b，新增：正整数
            var alarm_a = Number(position.baseInfo.alarm_smooth_a);
            ...
            if (Number(change_newVal) < alarm_a) {
              swal({
                title: "提示",
                text: "\u7B2C".concat(change_index + 1, "\u884C\u62A5\u8B66\u6295\u7968b\u4E0D\u5F97\u5C0F\u4E8E\u62A5\u8B66\u6295\u7968a"),
                button: "确认"
              });
              return false;
            }
          ...
          if (change_prop == "baseInfo.cc_mod_id_val") {
            //传感器类型
            var cc_mod_id_sources = position.baseInfo.cc_mod_id_sources;

            if (cc_mod_id_sources.indexOf(change_newVal) == -1) {
              swal({
                title: "提示",
                text: "\u7B2C".concat(change_index + 1, "\u884C\u4E0D\u5B58\u5728\u8BE5\u901A\u9053"),
                button: "确认"
              ...
              sources = position.baseInfo.cc_mod_id_sources;
              position.baseInfo.cc_mod_id_val = change_newVal;
              position.baseInfo.cc_mod_id = that.cc_mod_id_list[position.dgm_id][change_newVal].ch_id;
            }
          }
    ...
    value: function update_ch_source(position) {
      ...
      if (this.type == 200) {
        ...
        if (this.type == 10) {
          //工艺量测点需要更新关联组态
          var ccModList = this.cc_mod_id_list[position.dgm_id];
          cc_mod_source = Object.keys(ccModList);
          position.baseInfo.cc_mod_id_sources = cc_mod_source.sort(); //保存通道源
          //选取第一个通道为默认通道

          if (cc_mod_source.length == 0) {
            cc_mod_id_val = "";
          } else {
            cc_mod_id_val = cc_mod_source[0];
          }

          this.update_cc_channel(position, cc_mod_id_val);
        }
      }
    }
    /* 更改关联组态的通道 */
    ...
    key: "update_cc_channel",
    value: function update_cc_channel(position, cc_mod_id_val) {
      /* 所有测点-设置新的通道 */
      if (cc_mod_id_val === "" || cc_mod_id_val === undefined) {
        //没有关联组态的给个默认值
        position.baseInfo.cc_mod_id_val = "d1_mod_01";
        position.baseInfo.cc_mod_id = "0";
        position.baseInfo.cc_mod_id_sources = ["d1_mod_01"];
      } else {
        position.baseInfo.cc_mod_id_val = cc_mod_id_val;
        position.baseInfo.cc_mod_id = this.cc_mod_id_list[position.dgm_id][cc_mod_id_val].ch_id;
      }
    }
    /* 更改通道 */
    ...
    value: function set_channel(position, ch_name, del_pos) {
      ...
      if (ch_name === "" || ch_name === undefined) {
        ...
        if (position.pos_type == 17) {
          position.pos_id = Number(position.ch_id);
        }

        if (position.dgm_type == 7) {
          //tms的转速测点的转速比放到了通道设置里面，将转速测点的转速比置空
          position.baseInfo.speedRate = '';
        } else {
          position.baseInfo.speedRate = "1"; //其他的默认值为1
        }
        /* 如果是要删除通道的测点 */
    ...
    value: function update_frequency_spectral_sources(position) {
      ...
      this.filter_spectral_sources(position);
      Object(_pos_methods_js__WEBPACK_IMPORTED_MODULE_3__["computed__frequency_spectral"])(position);
    }
    /* 更改包络阶次的过滤器 */

  }, {
    key: "update_filter_frequency_spectral_sources",
    value: function update_filter_frequency_spectral_sources(position) {
      var frequency_object = Object(_pos_methods_js__WEBPACK_IMPORTED_MODULE_3__["frequencySources"])(position, position.baseInfo.filter);
      position.baseInfo.frequency_sources = Object.values(frequency_object);
      position.baseInfo.frequency_sources_key = Object.keys(frequency_object);
      position.baseInfo.spectral_sources = Object(_pos_methods_js__WEBPACK_IMPORTED_MODULE_3__["spectralSources"])(position.dgm_type, position.pos_type);
    ...
    /* 非超级用户V1采集器下的振动和包络测点谱线数需屏蔽一些选项 */

  }, {
    key: "filter_spectral_sources",
    value: function filter_spectral_sources(position) {
      // 针对非超级用户，v1采集器 谱线数下拉选项过滤
      var userType = parseInt(sessionStorage.getItem('userType')); // 振动测点，包络测点

      if (userType !== 0 && Number(position.dgm_type) === 1 && (Number(position.pos_type) === 3 || Number(position.pos_type) === 4)) {
        var spectral_source = ['400', '800', '1600', '3200', '6400', '12800', '25600', '51200', '102400'];
        var spectral_sources = [];
        var frequency = Number(position.baseInfo.frequency);

        if (Number(position.pos_type) === 3) {
          //振动测点
          for (var k = 0; k < spectral_source.length; k++) {
            var spectral = Number(spectral_source[k]);

            if (spectral > 1600 && frequency === 10) {
              continue;
            }

            if (spectral > 3200 && frequency === 20) {
              continue;
            }

            if (spectral > 6400 && frequency === 50) {
              continue;
            }

            if (spectral > 12800 && frequency === 100) {
              continue;
            }

            if (spectral > 25600 && frequency === 200) {
              continue;
            }

            if (spectral > 51200 && (frequency === 1000 || frequency === 5000)) {
              continue;
            }

            spectral_sources.push(spectral_source[k]);
          }
        }

        if (Number(position.pos_type) === 4) {
          //包络
          var filterType = Number(position.baseInfo.filter);

          for (var _k = 0; _k < spectral_source.length; _k++) {
            var _spectral = Number(spectral_source[_k]);

            switch (filterType) {
              case 0:
                if (_spectral > 1600 && frequency === 10) {
                  continue;
                }

                if (_spectral > 3200 && frequency === 20) {
                  continue;
                }

                if (_spectral > 6400 && frequency === 50) {
                  continue;
                }

                if (_spectral > 12800 && frequency === 100) {
                  continue;
                }

                if (_spectral > 25600 && frequency === 200) {
                  continue;
                }

                spectral_sources.push(spectral_source[_k]);
                break;

              case 1:
                if (_spectral > 400 && frequency === 10) {
                  continue;
                }

                if (_spectral > 800 && frequency === 20) {
                  continue;
                }

                if (_spectral > 1600 && frequency === 50) {
                  continue;
                }

                if (_spectral > 3200 && frequency === 100) {
                  continue;
                }

                if (_spectral > 6400 && frequency === 200) {
                  continue;
                }

                if (_spectral > 25600 && frequency === 500) {
                  continue;
                }

                if (_spectral > 51200 && frequency === 1000) {
                  continue;
                }
              ...
              case 2:
                if (_spectral > 400 && frequency === 50) {
                  continue;
                }

                if (_spectral > 800 && frequency === 100) {
                  continue;
                }

                if (_spectral > 1600 && frequency === 200) {
                  continue;
                }

                if (_spectral > 3200 && frequency === 500) {
                  continue;
                }

                if (_spectral > 6400 && frequency === 1000) {
                  continue;
                }

                if (_spectral > 12800 && frequency === 2000) {
                  continue;
                }

                if (_spectral > 51200 && frequency === 5000) {
                  continue;
                }
        ...
        position.baseInfo.spectral_sources = spectral_sources;
        position.baseInfo.spectral = spectral_sources[0];
      }
    }
    /* 长采样测点 */
...
var posMixin = {
  ...
  data: function data() {
    return {
      ...
      initList: {
        ...
        //存放倾覆或合成倾角带的晃度或者倾角测点
        task_idSources: {},
        //任务的统计列表 {dgm_id:{ch_id:{[task_id_sources],[indexs]}}}
        cc_mod_id_list: [] //工艺量测点的关联组态
  ...
  beforeRouteLeave: function beforeRouteLeave(to, from, next) {
    var macRoute = ['CementMac', 'RailMac', 'OilMac', 'CommonMac']; //机组设置的子路由

    if ((!$.isEmptyObject(this.changeList.changeData) || !$.isEmptyObject(this.changeList.addData)) && !macRoute.includes(to.name)) {
      swal({
        title: "警告",
  ...
  methods: {
    ...
    renderTable: function renderTable() {
      if (this.$route.path.indexOf('position') < 0) {
        //非当前页面不渲染
        return;
      }
      ...
      Object(_pos_methods__WEBPACK_IMPORTED_MODULE_2__["reRenderSelectOption"])(this.hot, this.data, this.type);
      /* 监听滚动事件 */
...
//# sourceMappingURL=8-c981ab8a.js.map