...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  beforeCreate: function beforeCreate() {//   sessionStorage.setItem('setCustomInfo',JSON.stringify({}))
    //   console.log('清空')
  },
  ...
  methods: {
    // 获取用户过滤设置信息
    getCustomInfo: function getCustomInfo(resolve) {
      var _this = this;
...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  mounted: function mounted() {
    ...
    this.$axios.post('getCurrentUserInfo', name).then(function (res) {
      ...
      that.userMsg = [{
        title: '用户名称',
        val: data.username ? data.username : name
      }, {
        title: '用户类型',
        val: that.setUserType(data.type)
      }];
      var tids = [];
      ...
      if (data.user_trees.length > 0) {
        ...
        _this.$store.commit('setTids', JSON.parse(sessionStorage.getItem('getTidList')));

        _this.$store.commit('setOrigTids', JSON.parse(sessionStorage.getItem('getTidList')));

        _this.$getApi.getAllTRoot(_this.$store.state.tids).then(function (res) {
      ...
      _this.$store.commit('setCurrentUserType', data.type.toString());
    })["catch"](function (err) {
      console.log(err);
      sessionStorage.setItem('tRootList', JSON.stringify([]));
    });
...
//# sourceMappingURL=25-fd20c9c4.js.map