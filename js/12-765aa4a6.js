...
function setHeaders(type) {
  ...
  if (type == 1 || type == 4) {
    header = ['组织名称', '机组名称', '测点名称', '测点类型', '数据类型', '工况#1', '工况#1预警（高）', '工况#1报警（高）', '工况#2', '工况#2预警（高）', '工况#2报警（高）', '工况#3', '工况#3预警（高）', '工况#3报警（高）', '工况#4', '工况#4预警（高）', '工况#4报警（高）', '其他工况', '其他工况预警（高）', '其他工况报警（高）'];
  } else if (type == 5) {
  ...
  switch (type) {
    ...
    case 5:
      ...
      col = [{
        data: "".concat(alarmBaseInfo, ".alarmInfo.name"),
        type: 'dropdown',
        source: alarmBaseInfo.valueSources,
        strict: true
      }, {
        data: "".concat(alarmBaseInfo, ".alarmInfo.status_1"),
        type: 'checkbox',
...
var tableData = /*#__PURE__*/function () {
  ...
  _createClass(tableData, [{
    ...
    value: function cells() {
      var that = this;
      return function (row, col, prop) {// let data =that.$this.data;
        // if(data.length == 0) return
        // let type = that.type;
        // let cellProperties = this;
        // /*下拉框表头集合*/
        // let selectProps = [
        //   'alarmBaseInfo.code_val'
        // ];
        ...
        //      let val = data[row].alarmBaseInfo.valueSources;
        ...
        //     return
        //   }
        //   // if(type == 5 && data[row] &&  data[row].alarmBaseInfo.alarmInfo.name!="无" && (prop=='alarmBaseInfo.alarmInfo.hhh_limit_5' || prop=='alarmBaseInfo.alarmInfo.hhh_limit_1' || prop=='alarmBaseInfo.alarmInfo.hhh_limit_2' || prop=='alarmBaseInfo.alarmInfo.hhh_limit_3' || prop=='alarmBaseInfo.alarmInfo.hhh_limit_4')){
        //   //   cellProperties.readOnly  = false;
        //   // }
        //   /* tmu的其他工况必须选中，不能不选*/
            ...
            if (flag === false) {
              ...
              hot.updateSettings({
                cells: function cells(row, col, prop) {
                  ...
                  if (prop == 'alarmBaseInfo.code_val') {
                    cellProperties.type = 'dropdown';
                    cellProperties.source = data[row].alarmBaseInfo.codeSources;
                  ...
                  if (col == 5) {
                    if (data[row].alarmBaseInfo.code == 2000) {
                      ...
                      cellProperties.source = data[row].alarmBaseInfo.alarmIndexSources;
                      cellProperties.strict = true;
                    } else {
                      cellProperties.prop = "alarmBaseInfo.vib_k_hh";
                      cellProperties.type = 'text';
      ...
      return function (changes, source) {
        ...
        if (changes) {
          changes.forEach(function (change) {
            ...
            if (that.type == 1 || that.type == 4 || that.type == 5) {
  ...
  if (res.msg === 1 && res.alarm_list && res.alarm_list.length > 0) {
    if (type == 1) {
      ...
      for (var i = 0; i < origionData.length; i++) {
        ...
        posType = parseInt(origionData[i].pos_type); // if(!overFlag){
      ...
      res.alarm_list.forEach(function (machine) {
        alarmBaseInfo = {
          ...
          pos_type_name: Object(_alarm_methods_js__WEBPACK_IMPORTED_MODULE_0__["posTypeTrans"])(machine.pos_type),
          baseInfo: machine.baseInfo,
          code_val: Object(_alarm_methods_js__WEBPACK_IMPORTED_MODULE_0__["getCodeName"])(machine.baseInfo.alarmList[0].code),
          code: machine.baseInfo.alarmList[0].code,
          codeSources: Object.values(codeSource),
          codeSources_key: Object.keys(codeSource),
          alarm_threshold_index: machine.baseInfo.alarmList[0].alarm_threshold_index,
          alarm_threshold_index_val: Object(_alarm_methods_js__WEBPACK_IMPORTED_MODULE_0__["alarmIndex"])(machine.baseInfo.alarmList[0].alarm_threshold_index),
          alarmIndexSources: Object.values(alarmIndexSource),
          alarmIndexSources_key: Object.keys(alarmIndexSource),
          vib_k_hh: machine.baseInfo.alarmList[0].vib_k_hh,
          alarm_counter: machine.baseInfo.alarmList[0].alarm_counter,
          columnObj: machine.baseInfo.alarmList[0].code == 2000 ? Object(_alarm_methods_js__WEBPACK_IMPORTED_MODULE_0__["alarmIndex"])(machine.baseInfo.alarmList[0].alarm_threshold_index) : machine.baseInfo.alarmList[0].vib_k_hh
        };
        data.push({
          alarmBaseInfo: alarmBaseInfo
        });
      ...
      for (var _i = 0; _i < origionData.length; _i++) {
        origionData[_i].statusInfo = [];
        origionData[_i].valueSource = {};

        if (origionData[_i].baseInfo && origionData[_i].baseInfo.length > 0) {
          origionData[_i].statusInfo = origionData[_i].baseInfo;
        }

        origionData[_i].codeArr = {};
        _posType = parseInt(origionData[_i].pos_type);
        origionData[_i].statusInfo.length = 1;
        origionData[_i].valueSource = Object(_alarm_methods_js__WEBPACK_IMPORTED_MODULE_0__["getCodeObj"])(Object(_alarm_methods_js__WEBPACK_IMPORTED_MODULE_0__["getFlagType"])(_posType), 0, origionData[_i].tt_pos_loc, origionData[_i].dgm_type);
        origionData[_i].valueSource["无"] = "";

        for (var _m = 0; _m < origionData[_i].statusInfo.length; _m++) {
          if (origionData[_i].statusInfo[_m] == undefined) {
            origionData[_i].statusInfo[_m] = {
              code: "",
              h_limit_1: "3",
              h_limit_2: "3",
              h_limit_3: "3",
          ...
          if (origionData[_i].statusInfo[_m].code != "") {
            origionData[_i].statusInfo[_m].name = Object(_alarm_methods_js__WEBPACK_IMPORTED_MODULE_0__["getCodeName"])(origionData[_i].statusInfo[_m].code); //根据code得到特征值的名称
          }

          alarmBaseInfo = {
            mac_pos_id: origionData[_i].mac_id + "_" + origionData[_i].pos_id + "_" + origionData[_i].pos_type,
            t_id: origionData[_i].t_id,
            t_name: origionData[_i].t_name,
            mac_id: origionData[_i].mac_id,
            mac_me: origionData[_i].mac_me,
            m_type: origionData[_i].m_type,
            pos_id: origionData[_i].pos_id,
            dgm_type: origionData[_i].dgm_type,
            pos_name: origionData[_i].pos_name,
            pos_type: origionData[_i].pos_type,
            pos_type_name: Object(_alarm_methods_js__WEBPACK_IMPORTED_MODULE_0__["posTypeTrans"])(origionData[_i].pos_type),
            baseInfo: origionData[_i].baseInfo,
            alarmInfo: origionData[_i].statusInfo[_m],
            valueSources: Object.keys(origionData[_i].valueSource),
            valueSources_key: Object.values(origionData[_i].valueSource),
            codeArr: origionData[_i].valueSource
          };
          data.push({
            alarmBaseInfo: alarmBaseInfo
          });
      ...
      for (var _i2 = 0; _i2 < origionData.length; _i2++) {
        origionData[_i2].statusInfo = [];
        origionData[_i2].valueSource = {};

        if (origionData[_i2].baseInfo && origionData[_i2].baseInfo.length > 0) {
          origionData[_i2].statusInfo = origionData[_i2].baseInfo;
        }

        origionData[_i2].codeArr = {};
        _posType2 = parseInt(origionData[_i2].pos_type);
        origionData[_i2].statusInfo.length = 15;
        origionData[_i2].valueSource = Object(_alarm_methods_js__WEBPACK_IMPORTED_MODULE_0__["getCodeObj"])(Object(_alarm_methods_js__WEBPACK_IMPORTED_MODULE_0__["getFlagType"])(_posType2), 2, origionData[_i2].tt_pos_loc, origionData[_i2].dgm_type);
        origionData[_i2].valueSource["无"] = "";

        for (var _m2 = 0; _m2 < origionData[_i2].statusInfo.length; _m2++) {
          if (origionData[_i2].statusInfo[_m2] == undefined) {
            origionData[_i2].statusInfo[_m2] = {
              code: "",
              h_limit_1: "3",
              h_limit_2: "3",
              h_limit_3: "3",
              ...
              hh_limit_4: "5",
              hh_limit_5: "5",
              hhh_limit_1: origionData[_i2].dgm_type == 7 ? "7" : "",
              hhh_limit_2: origionData[_i2].dgm_type == 7 ? "7" : "",
              hhh_limit_3: origionData[_i2].dgm_type == 7 ? "7" : "",
              hhh_limit_4: origionData[_i2].dgm_type == 7 ? "7" : "",
              hhh_limit_5: origionData[_i2].dgm_type == 7 ? "7" : "",
              name: "无",
              status: "0",
              status_1: "0",
              status_2: "0",
          ...
          if (origionData[_i2].statusInfo[_m2].code != "") {
            origionData[_i2].statusInfo[_m2].name = Object(_alarm_methods_js__WEBPACK_IMPORTED_MODULE_0__["getCodeName"])(origionData[_i2].statusInfo[_m2].code); //根据code得到特征值的名称
          }

          alarmBaseInfo = {
            mac_pos_id: origionData[_i2].mac_id + "_" + origionData[_i2].pos_id + "_" + origionData[_i2].pos_type,
            t_id: origionData[_i2].t_id,
            t_name: origionData[_i2].t_name,
            mac_id: origionData[_i2].mac_id,
            mac_me: origionData[_i2].mac_me,
            m_type: origionData[_i2].m_type,
            pos_id: origionData[_i2].pos_id,
            dgm_type: origionData[_i2].dgm_type,
            pos_name: origionData[_i2].pos_name,
            pos_type: origionData[_i2].pos_type,
            pos_type_name: Object(_alarm_methods_js__WEBPACK_IMPORTED_MODULE_0__["posTypeTrans"])(origionData[_i2].pos_type),
            baseInfo: origionData[_i2].baseInfo,
            alarmInfo: origionData[_i2].statusInfo[_m2],
            valueSources: Object.keys(origionData[_i2].valueSource),
            valueSources_key: Object.values(origionData[_i2].valueSource),
            codeArr: origionData[_i2].valueSource
          };
          data.push({
            alarmBaseInfo: alarmBaseInfo
          });
    ...
    for (var key in oldAlarm.alarmBaseInfo.codeArr) {
      ...
      for (var j = 0; j < changeArr.length; j++) {
        if (code != '' && code == changeArr[j].alarmBaseInfo.alarmInfo.code) {
          ...
          if (type == 5) {
            //tms报警
            baseInfoObj.hhh_limit_1 = "7";
            baseInfoObj.hhh_limit_2 = "7";
            baseInfoObj.hhh_limit_3 = "7";
            baseInfoObj.hhh_limit_4 = "7";
            baseInfoObj.hhh_limit_5 = "7";
          }
        }
      }
...
var alarmMixin = {
  ...
  methods: {
    getListData: function getListData(type) {
      ...
      this.$getApi.queryAllAlarmInfobyType(type, this.$store.state.tids).then(function (res) {
        loadTime = true;
    ...
    renderTable: function renderTable() {
      ...
      if (this.$route.path.indexOf('alarm') < 0) {
        //非当前页面不渲染
        return;
      }
      ...
      cellMeta.forEach(function (item) {
        if (item.prop == "alarmBaseInfo.alarmInfo.name") {
          ...
          for (var i = 0; i < rows; i++) {
            _this3.hot.setCellMeta(i, item.col, "source", _this3.data[_this3.hot.toPhysicalRow(i)].alarmBaseInfo.valueSources);
          }
        }
        ...
        if (_this3.type == 2) {
          ...
          for (var _i = 0; _i < rows; _i++) {
            var rowNum = _this3.hot.toPhysicalRow(_i);

            if ((_this3.data[rowNum].alarmBaseInfo.dgm_type == 2 || _this3.data[rowNum].alarmBaseInfo.dgm_type == 1) && _this3.data[rowNum].alarmBaseInfo.pos_type == 5) {
              _this3.hot.setCellMeta(_i, item.col, "readOnly", true);
            }

            if (_this3.data[rowNum].alarmBaseInfo.pos_type == 5 && (item.prop == 'alarmBaseInfo.baseInfo.ll_limit' || item.prop == 'alarmBaseInfo.baseInfo.l_limit')) {
              _this3.hot.setCellMeta(_i, item.col, "readOnly", true);
            }
          }
        }
        ...
        if (_this3.type == 3) {
          ...
          if (item.col == 5) {
            ...
            for (var _i2 = 0; _i2 < rows; _i2++) {
              var _rowNum = _this3.hot.toPhysicalRow(_i2);

              if (_this3.data[_rowNum].alarmBaseInfo.code == 2000) {
                _this3.hot.setCellMeta(_i2, item.col, "prop", "alarmBaseInfo.alarm_threshold_index_val");

                _this3.hot.setCellMeta(_i2, item.col, "editor", "dropdown");

                _this3.hot.setCellMeta(_i2, item.col, "strict", true);

                _this3.hot.setCellMeta(_i2, item.col, "source", _this3.data[_this3.hot.toPhysicalRow(_i2)].alarmBaseInfo.alarmIndexSources);
              } else {
                _this3.hot.setCellMeta(_i2, item.col, "prop", "alarmBaseInfo.vib_k_hh");

                _this3.hot.setCellMeta(_i2, item.col, "type", "text");
              }
            }
          }

          if (item.prop == "alarmBaseInfo.code_val") {
            //数据类型
            for (var _i3 = 0; _i3 < rows; _i3++) {
              _this3.hot.setCellMeta(_i3, item.col, "source", _this3.data[_this3.hot.toPhysicalRow(_i3)].alarmBaseInfo.codeSources);
            }
          }
        }
      });
...
//# sourceMappingURL=12-765aa4a6.js.map